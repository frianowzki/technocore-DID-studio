...
try {
  ...
  const expression = `(async () => {
    ...
    const signedOnly = {
      ...
      fallbackNoteBeforePublish: document.querySelector('#signed-result .warning').textContent.trim(),
    };
    for (let attempt = 0; attempt < 50 && document.querySelector('#feed-list').getAttribute('aria-busy') === 'true'; attempt += 1) {
    ...
    const seedBackupStatus = document.querySelector('#status').textContent;
    const seedBackupFormReset = document.querySelector('#seed-backup-passphrase').value === '' && document.querySelector('#seed-backup-confirmation').value === '' && !document.querySelector('#seed-backup-check').checked;
    ...
    document.querySelector('#restore-form').requestSubmit();
    for (let attempt = 0; attempt < 50 && document.querySelector('#seed-backup-fieldset').disabled; attempt += 1) {
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    ...
    const fallbackDisabledAfterPublish = document.querySelector('#open-request').disabled;
    const copyUrlDisabledAfterPublish = document.querySelector('#copy-url').disabled;
    const consumedRequestUrlCleared = document.querySelector('#request-url').value === '';
    const publishedSignedState = document.querySelector('#result-state').textContent;
    const fallbackNoteAfterPublish = document.querySelector('#signed-result .warning').textContent.trim();
    ...
    const ownedRecordsSnapshot = {
      ...
      introductionText: document.querySelector('#owned-introduction-text')?.textContent ?? '',
      contributionRoom: document.querySelector('#owned-contribution-room')?.textContent ?? '',
      contributionSequence: document.querySelector('#owned-contribution-seq')?.textContent ?? '',
      contributionNonce: document.querySelector('#owned-contribution-nonce')?.textContent ?? '',
      contributionText: document.querySelector('#owned-contribution-text')?.textContent ?? '',
      publicFeedHeading: document.querySelector('#public-feed-title')?.textContent ?? '',
    };
    const downloads = [];
    ...
    const darkPaper = getComputedStyle(document.documentElement).getPropertyValue('--paper').trim();
    ...
    return {
      ...
      initialTheme,
      themeAfterToggle,
      savedTheme: localStorage.getItem('technocore-theme'),
      lightPaper,
      darkPaper,
      themeBesideGithub: Boolean(themeBounds && Math.abs(themeBounds.right - githubBounds.left) <= 12),
      initialBackupName,
      ...
      lobbyRecord,
      fallbackDisabledAfterPublish,
      copyUrlDisabledAfterPublish,
      consumedRequestUrlCleared,
      publishedSignedState,
      fallbackNoteAfterPublish,
      preparedRoom,
      preparedMessage,
      ...
      feedStatus: document.querySelector('#feed-status').textContent.startsWith('Live —') ? 'live' : document.querySelector('#feed-status').textContent,
